...
import PacienteDetallesItem from "./PacienteDetallesItem";


interface PacienteDetallesProps {
  paciente: Pacientes;
}

export default function PacienteDetalles({ paciente }: PacienteDetallesProps) {
  
  const { eliminarPaciente, obtenerPacienteId} = usePacientesStore()
  return (
    <div className="mx-5 my-10 px-5 py-10 bg-white shadow-md rounded-xl">
      <PacienteDetallesItem label="ID" data={paciente.id} />
      <PacienteDetallesItem label="Nombre" data={paciente.name} />
      <PacienteDetallesItem label="Propietario" data={paciente.caretaker} />
      <PacienteDetallesItem label="Email" data={paciente.email} />
      <PacienteDetallesItem label="Fecha" data={paciente.date.toString()} />
      <PacienteDetallesItem label="Sintomas" data={paciente.sintomas} />

      <div className="flex justify-center gap-3 mt-10">
        <button className="flex gap-2 font-mono transform transition duration-300 hover:scale-105" onClick={() => obtenerPacienteId(paciente.id)}>
          Editar
          <svg
            width={20}
            height={20}
        ...
        <button className="flex gap-2 font-mono transform transition duration-300 hover:scale-105" onClick={() => eliminarPaciente(paciente.id)}>
          Eliminar
          <svg
          width={20}
          height={20}
